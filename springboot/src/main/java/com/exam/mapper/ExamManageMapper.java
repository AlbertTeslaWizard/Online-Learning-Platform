...
public interface ExamManageMapper {
    ...
    @Select("select* from exam_manage where Source = '练习集' ")
    IPage<ExamManage> findPart(Page page);
    ...
    @Update("update exam_manage set description = #{description},source = #{source},paperId = #{paperId}," +
            ...
            "type = #{type},tips = #{tips}, isPost = #{isPost} where examCode = #{examCode}")
    int update(ExamManage exammanage);

    @Options(useGeneratedKeys = true, keyProperty = "examCode")
    @Insert("insert into exam_manage(examCode, description,source,paperId,examDate,totalTime,grade,term,major,institute,totalScore,type,tips,isPost)" +
            " values(#{examCode}, #{description},#{source},#{paperId},#{examDate},#{totalTime},#{grade},#{term},#{major},#{institute},#{totalScore},#{type},#{tips},#{isPost})")
